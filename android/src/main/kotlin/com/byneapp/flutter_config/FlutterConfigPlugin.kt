...
class FlutterConfigPlugin(private val context: Context? = null) : FlutterPlugin, MethodCallHandler {
  ...
  companion object {
    ...
    fun registerWith(messenger: BinaryMessenger, context: Context) {
      val channel = MethodChannel(messenger, "flutter_config")
      channel.setMethodCallHandler(FlutterConfigPlugin(context))
    }
  }
  ...
  private fun loadEnvVariables(): Map<String, Any?> {
    ...
    try {
      ...
      val resId =
              context.resources.getIdentifier("build_config_package", "string", context.packageName)
      ...
      clazz.declaredFields.forEach { variables += it.name to extractValue(it) }
